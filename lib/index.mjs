...
const CreateLookups = alphabet => {
  ...
  const SAFE_BASE_MAP_LOOKUP = new Map();
  for (let i = 0; i < 64; i++) {
    ...
    MapSet(SAFE_ALPHABET_LOOKUP, char, i);
    MapSet(SAFE_BASE_MAP_LOOKUP, charCode, i);
  }
  return {
    ALPHABET_LOOKUP,
    BASE_MAP,
    BASE_MAP_LOOKUP,
    SAFE_ALPHABET_LOOKUP,
    SAFE_BASE_MAP_LOOKUP
  };
}
...
const GetSafeAlphabetCharIndex = FunctionBind(MapPrototypeGet, SAFE_ALPHABET_LOOKUP);
const GetSafeAlphabetCharIndexByCode = FunctionBind(MapPrototypeGet, SAFE_BASE_MAP_LOOKUP);
...
ArrayForEach([
  ...
  const charCode = StringCharCodeAt(alias);
  MapSet(SAFE_ALPHABET_LOOKUP, alias, charIndex);
  MapSet(SAFE_BASE_MAP_LOOKUP, charCode, charIndex);
});
...
const Decode = (encodedString, ignorePadding, allowConcatenation, allowSafe) => {
  const length = encodedString.length;
  if (!length) {
    return '';
  }
  const getAlphabetCharIndexByCode = allowSafe ? GetSafeAlphabetCharIndexByCode : GetAlphabetCharIndexByCode;
  let result = '';
  let position = 0;
...
const DecodeToBytes = (encodedString, ignorePadding, allowConcatenation, allowSafe) => {
  const length = encodedString.length;
  if (!length) {
    return new Uint8Array(0);
  }
...
const DecodeBytes = (buffer, ignorePadding, allowConcatenation, allowSafe) => {
  const source = IsUint8Array(buffer) ? buffer : new Uint8Array(buffer);
  const length = TypedArrayLength(source);
  if (!length) {
    return new Uint8Array(0);
...
const DecodeBytesToString = (buffer, ignorePadding, allowConcatenation, allowSafe) => {
  const source = IsUint8Array(buffer) ? buffer : new Uint8Array(buffer);
  const length = TypedArrayLength(source);
  if (!length) {
    return '';
...
const DecodeText = (encodedString, ignorePadding, allowConcatenation, allowSafe) => {
  const buffer = DecodeToBytes(encodedString, ignorePadding, allowConcatenation, allowSafe);
  return UTF8Decode(buffer);
}

const DecodeBytesToText = (buffer, ignorePadding, allowConcatenation, allowSafe) => {
  const bytes = DecodeBytes(buffer, ignorePadding, allowConcatenation, allowSafe);
...
const DecodeInt = (encodedInteger, allowSafe) => {
  const length = encodedInteger.length;
  if (!length) {
    return NaN;
  }
  const getAlphabetCharIndex = allowSafe ? GetSafeAlphabetCharIndex : GetAlphabetCharIndex;
  let leadingZeros = 0;
...
export const decode = (encodedString, options) => {
  ...
  if (options !== undefined) {
    ...
    allowSafe = !!options.allowSafe;
  }
  return Decode($encodedString, ignorePadding, allowConcatenation, allowSafe);
}
...
export const decodeToBytes = (encodedString, options) => {
  ...
  return DecodeToBytes($encodedString, ignorePadding, allowConcatenation, allowSafe);
}
...
export const decodeBytes = (buffer, options) => {
  ...
  return DecodeBytes(buffer, ignorePadding, allowConcatenation, allowSafe);
}
...
export const decodeBytesToString = (buffer, options) => {
  ...
  return DecodeBytesToString(buffer, ignorePadding, allowConcatenation, allowSafe);
}
...
export const decodeText = (encodedString, options) => {
  ...
  return DecodeText($encodedString, ignorePadding, allowConcatenation, allowSafe);
}
...
export const decodeBytesToText = (buffer, options) => {
  ...
  return DecodeBytesToText(buffer, ignorePadding, allowConcatenation, allowSafe);
}
...
export const decodeInt = (encodedInteger, options) => {
  ...
  return DecodeInt($encodedInteger, allowSafe);
}

export const Base64 = ObjectCreate(ObjectPrototype, {
  encode: {
    value: encode
  },
  encodeToBytes: {
...
if (BigInt) {
  ...
  const DecodeBigInt = (encodedInteger, allowSafe) => {
    const length = encodedInteger.length;
    if (!length) {
  ...
  decodeBigInt = (encodedInteger, options) => {
    const $encodedInteger = ToString(encodedInteger);
    let allowSafe = false;
    if (options !== undefined) {
      RequireOptionsObject(options);
    ...
    return DecodeBigInt($encodedInteger, allowSafe);
  }
